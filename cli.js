...
console.log(
  `echo host listening on ${host.port}, eom=${JSON.stringify(host.eom)}`,
);
...
async function handleCommand(line) {
  ...
  switch (cmd) {
    case 'port': {
      ...
      if (!args[0] || Number.isNaN(n))
        return closeCommandInput('{red-fg}usage: port <number>{/red-fg}');
    ...
    case 'eom': {
      if (!args[0])
        return closeCommandInput('{red-fg}usage: eom <char>{/red-fg}');
      host.eom = args[0];
      setEOM(host.eom);
      return closeCommandInput(
        `{cyan-fg}eom -> ${JSON.stringify(host.eom)}{/cyan-fg}`,
      );
    }
    case 'sub': {
      ...
      if (action === 'add') {
        if (!address || !portArg)
          return closeCommandInput(
            '{red-fg}usage: sub add <address> <port>{/red-fg}',
          );
        host.addSubscriber(address, Number(portArg));
      } else if (action === 'remove') {
        if (!address || !portArg)
          return closeCommandInput(
            '{red-fg}usage: sub remove <address> <port>{/red-fg}',
          );
        host.removeSubscriber(address, Number(portArg));
      } else {
        return closeCommandInput(
          '{red-fg}usage: sub <add|remove> <address> <port>{/red-fg}',
        );
      }
      setSubscribers(host.listSubscribers());
      return closeCommandInput(
        `{cyan-fg}subscribers: ${host.listSubscribers().length}{/cyan-fg}`,
      );
    }
    case 'q':
    case 'quit':
